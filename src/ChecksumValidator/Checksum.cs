...
namespace ChecksumValidator
{
    ...
    {
        public static async Task<string> ComputeChecksumAsync(string sourceFile, ChecksumMethod method, Action<decimal> progressNotifier)
        {
            string checksum = "";

            await Task.Run(() =>
            {
                checksum = ComputeChecksum(sourceFile, method, progressNotifier);
            });
