...
namespace ChecksumValidator.Utilities
{
    /// <summary>
            ...
            TraverseExceptionTree(
                ...
                delegate(KeyValuePair<KeyValuePair<int, bool>, Exception> info) //exception detail printer
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (info.Value is DbException dbEx)
                            {
                                linePrinter.PrintPrefix();
                                builder.Append("SqlException.Message: ");
                                builder.Append(dbEx.Message);
                                builder.Append(" (0x");
                                builder.Append(dbEx.ErrorCode.ToString("X8", CultureInfo.InvariantCulture));
                                builder.AppendLine(")");
                                builder.AppendLine();
                            }
                            else
                            {
                                ;
                                if (info.Value is ExternalException eEx)
                                {
                                    linePrinter.PrintPrefix();
